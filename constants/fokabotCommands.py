...
def report(fro, chan, message):
    ...
    try:
        ...
        glob.db.execute("INSERT INTO reports (id, from_uid, to_uid, reason, chatlog, time) VALUES (NULL, %s, %s, %s, %s, %s)", [userUtils.getID(fro), targetID, "{reason} - ingame {info}".format(reason=reason, info="({})".format(additionalInfo) if additionalInfo != None else ""), chatlog, int(time.time())])
...
def multiplayer(fro, chan, message):
    ...
    def mpStart():
        ...
        for i, slot in enumerate(_match.slots):
            if slot.status != slotStatuses.READY and slot.user != None:

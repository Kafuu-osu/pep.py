...
class match:
    ...
    def getMatchData(self, censored = False):
        ...
        for i in range(0,16):
            if self.slots[i].user != None and self.slots[i].user in glob.tokens.tokens:
    ...
    def toggleSlotLocked(self, slotID):
        ...
        if self.slots[slotID].user != None and self.slots[slotID].user in glob.tokens.tokens:
    ...
    def allPlayersCompleted(self):
        ...
        for i in range(0,16):
            if self.slots[i].user != None and self.slots[i].status == slotStatuses.PLAYING:
    ...
    def getUserSlotID(self, userID):
        ...
        """
        for i in range(0,16):
            if self.slots[i].user != None and self.slots[i].user in glob.tokens.tokens and glob.tokens.tokens[self.slots[i].user].userID == userID:
                return i
        return None
    ...
    def userChangeSlot(self, userID, newSlotID):
        ...
        if self.slots[newSlotID].user != None or self.slots[newSlotID].status != slotStatuses.FREE:
            return False
    ...
    def checkTeams(self):
        ...
        firstTeam = -1
        for i in range(0,16):
            if self.slots[i].user != None and (self.slots[i].status & slotStatuses.NO_MAP) == 0:
                if firstTeam == -1:
